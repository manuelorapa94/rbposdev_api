...
function saveAccessToken (done) {
  ...
  if(!this.user) {
    return done(error('server_error', false, null));
  }

  this.model.saveAccessToken(accessToken, this.client.clientId, expires, this.user, function (err,ress) {
    if (err) {
...
function sendResponse (done) {
  ...
  if (!this.user) {
    ...
    response.tenant_id = this.user.tenant_id;
    response.user_id = this.user.user_id;
    response.first_name = this.user.FirstName;
    response.last_name = this.user.LastName;
    response.current_location_id = this.user.current_location_id;
  }
