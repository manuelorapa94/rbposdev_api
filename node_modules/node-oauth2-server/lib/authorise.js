...
function getBearerToken (done) {
  var headerToken = this.req.get('Authorization'),
    ...
    tenantid = this.req.query.tenant_id,
    userid = this.req.query.user_id;
...
function checkToken (done) {
  var self = this;
  var params = {
    tenant_id: this.req.query.tenant_id,
    user_id: this.req.query.user_id,
    bearerToken: this.bearerToken
  }
  this.model.getAccessToken(params, function (err, token) {
    ...
    self.req.user = token.user ? token.user : { id: token.user.id };
    self.req.tenant = token.tenant ? token.tenant : { id: token.tenant.id };
    done();
  });
}
